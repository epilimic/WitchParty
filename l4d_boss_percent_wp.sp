...
public Action:SaveBossFlows(Handle:timer)
{
	...
	if (readyUpIsAvailable)
	{
		...
		if (iWitchPercent && iTankPercent)
			Format(readyString, sizeof(readyString), "Tank: %d%%, Witch: Party!", iTankPercent, iWitchPercent);
		else if (iTankPercent)
			Format(readyString, sizeof(readyString), "Tank: %d%%, Witch: Party!", iTankPercent);
		else if (iWitchPercent)
			Format(readyString, sizeof(readyString), "No Flow Tank, Witch: Party!", iWitchPercent);
		else
			Format(readyString, sizeof(readyString), "No Flow Tank, Witch: Party!");
...
stock PrintBossPercents(client)
{
	...
	{
		if (iTankPercent)
			PrintToChat(client, "\x01Tank: [\x05%d%%\x01]", iTankPercent);
		else
			PrintToChat(client, "\x01Tank: [\x05No Flow Tank\x01]");
	}

	if(GetConVarBool(hCvarWitchPercent))
	{
		if (iWitchPercent)
			PrintToChat(client, "\x01Witch: [\x05Party!\x01]", iWitchPercent);
		else
			PrintToChat(client, "\x01Witch: [\x05Party!\x01]");
	}
}
