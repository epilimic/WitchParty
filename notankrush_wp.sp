...
FreezePoints( bool:show_message = false ) {
    if ( !bTankAlive ) {
        ...
        if ( show_message ) PrintToChatAll("\x01[\x05Witch Party!\x01] Tank is up! \x05Distance points frozen!");
...
UnFreezePoints( bool:show_message = false ) {
    if ( bTankAlive ) {
        if ( show_message ) PrintToChatAll("\x01[\x05Witch Party!\x01] Tank is dead! \x05Unfreezing distance points!");
